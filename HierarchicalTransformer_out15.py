...
from BiGCN_Dataloader_out import BiGCNTwitterSet, FastBiGCNDataset, MetaMCMCDataset, load_data_all, load_data_twitter15
from BaseModel.modeling_bert import *
from transformers.models.bert import BertConfig, BertTokenizer
import torch, torch.nn as nn
from typing import List
from torch.utils.data import Dataset
from BiGCN_Dataloader_out import load_data
...
class BiGCNTrainer(RumorBaseTrainer):
    ...
    def fit(self, model: TwitterTransformer, train_set, dev_eval=None, test_eval=None, batch_size=5,
        ...
        optim.zero_grad()
        loss_list = []
        # model.load_model("./twitter15/BiGCN_twitter15_0.807.pkl")
        # model.load_model("./twitter15/BiGCN_twitter15_0.78.pkl")
        # model.load_model("./twitter16/BiGCN_twitter16_0.845.pkl")
...
class MetaBiGCNTrainer(BiGCNTrainer, MetaLearningFramework):
    ...
    def meta_learning(self, model: TwitterTransformer, train_set, meta_dev,
        ...
        train_step = 0
        # model.load_model("./twitter15/BiGCN_twitter15_0.78.pkl")
        model.load_model("./twitter16/BiGCN_twitter16_0.757.pkl")
        for epoch in range(max_epochs):
            print("epoch", epoch)
...
class MCMCBiGCNTrainer(MetaBiGCNTrainer, FastBiGCNDataset):
    ...
    def dataset2dataloader2(self, model: TwitterTransformer, dataset: MetaMCMCDataset, shuffle=False, batch_size=32):
        ...
        self.MCMC_Augmentation(dataset, model, lm_generator, max_step=400, batch_idxs=batch_idxs,
                               batch_size=32, temperature=0.1,
                               tr_data_len=tr_data_len)
    ...
    def validate_cpt(self,model: TwitterTransformer,test_eval: TransformerEvaluator):
        ...
        model.load_model("./twitter15/BiGCN_twitter15.pkl")
        test_acc = test_eval(model)
        print("test_acc",test_acc)

def trainset2trainloader(dataset: MetaMCMCDataset, shuffle=False, batch_size=32):
    return dataset2dataloader(dataset, shuffle, batch_size)

def dataset2dataloader(dataset: BiGCNTwitterSet, shuffle=False, batch_size=32):
    if shuffle:
        idxs = random.sample(range(len(dataset)), len(dataset)) * 2
    else:
        idxs = [*range(len(dataset))] * 2
...
def collate_fn(items):
    ...
    print("len(sentences)",len(sentences))
    print("sum(num_nodes",sum(num_nodes))
    print("--------------------------------------------------------")
...
if __name__ == '__main__':
    ...
    data_dir2 = r"../da-mstf/t1516_text/preprocessed_data/"

    # events_list = ['charliehebdo', 'ferguson', 'germanwings-crash', 'ottawashooting', 'sydneysiege',"twitter15","twitter16"]
    events_list = ['charliehebdo', 'ferguson', 'germanwings-crash', 'ottawashooting', 'sydneysiege', "twitter15"]
    domain_ID = 5
    source_events = []
    target_events = []
    ...
    trainer = MCMCBiGCNTrainer(log_dir, None, model_rename=True, beam_K=3, batch_dir=f"./aug_dir6")
    # trainer.fit(model1, tr, dev_eval, te_eval, batch_size=32, grad_accum_cnt=1, learning_rate=2e-5,
    #             max_epochs=20,
    #             model_file=os.path.join(log_dir, f'BiGCN_{test_event_name}.pkl'))

    # trainer.dataset2dataloader2(model1, tr, shuffle=True, batch_size=32)
    # trainer.meta_learning(model1, train_set=tr, meta_dev=meta_dev, dev_eval=dev_eval, test_eval=te_eval, batch_size=32,
