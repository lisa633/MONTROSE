...
if __name__ == '__main__':
    ...
    if os.path.exists(f"./{test_event_name}/BiGCN_{test_event_name}.pkl"):
        model.load_model(f"./{test_event_name}/BiGCN_{test_event_name}.pkl")
    else:
        ...
        if os.path.exists(f"./{test_event_name}/BiGCN_{test_event_name}.pkl"):
            ...
            model.save_model(f"./{test_event_name}/BiGCN_{test_event_name}.pkl")    

    trainer = DgMSTF_Trainer(random_seed=10086, log_dir=logDir, suffix=f"{test_event_name}_FS{fewShotCnt}",model_file=f"./DgMSTF_{test_event_name}_FS{fewShotCnt}.pkl", domain_num=5,class_num=2, temperature=0.05, learning_rate=2e-5, batch_size=32, epsilon_ball=5e-5,gStep=5, Lambda=0.1, D_lr=2e-3, valid_every=20, dStep=20) 
    ...
    if os.path.exists(f"./DomainDiscriminator_{test_event_name}.pkl"):
        trainer.domain_discriminator.load_state_dict(
            torch.load(f"./DomainDiscriminator_{test_event_name}.pkl")
        )
    else:
        for epoch in range(3):
            trainer.optimizeDiscriminator(model, source_domain, unlabeled_target, max_step=500)
        torch.save(trainer.domain_discriminator.state_dict(), f"./DomainDiscriminator_{test_event_name}.pkl")
