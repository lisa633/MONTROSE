...
#     gen_target = copy.deepcopy(source_domain)
    ...
    for i,d_ID in enumerate(source_domain.data_ID[2300:2450]):
...
#     all_gen_target.dataclear()
    ...
    event_dir = os.path.join(data_dir1,"qwen_gen_from_source","2",test_event_name)
    print(event_dir)
    if os.path.exists(f"../../autodl-tmp/data/pheme-rnr-dataset/qwen_gen_from_source/2"):
        gen_target.Caches_Data(event_dir)
    else:
        os.mkdir(f"../../autodl-tmp/data/pheme-rnr-dataset/qwen_gen_from_source/2")
        gen_target.Caches_Data(event_dir)
