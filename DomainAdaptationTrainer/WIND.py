...
class WindTrainer(InstanceReweightingV4, BaseTrainer):
    def __init__(self, class_num, log_dir, suffix, weight_eta=0.1, lr4model=2e-2,
                 coeff4expandset=1.0, max_few_shot_size=20, Inner_BatchSize=5, meta_step=5,model_file='',temperature="",learning_rate=2e-5,
                         batch_size=32, Lambda=0.1):
    ...
    def dataIter(self, OOD_Set, InD_Set=None, batch_size=32):
        ...
        if InD_Set is None:
            ...
            l_len = len(l_idxs)
        # print("not hasattr(self, 'collate_fn')",hasattr(self, 'collate_fn'))

        if not hasattr(self, 'collate_fn'):
            # collate_fn = OOD_Set.collate_raw_batch
            collate_fn =OOD_Set.collate_fn
        else:
    ...
    def Training(self, model:nn.Module, train_set:Dataset, valid_set:Dataset,
                 ...
                 model_file="./tmp.pkl",test_label="",learning_rate=2e-5):
        self.lr4model=learning_rate
