...
#     gen_target = copy.deepcopy(source_domain)
    ...
    for i,d_ID in enumerate(source_domain.data_ID[:100]):
        temp_dict = source_domain.data[d_ID]
        temp_dict["text"] = [s.split(" ") for s in temp_dict["sentence"]]
#         print(temp_dict["sentence"])
        for s in temp_dict["sentence"]:
            source_sentence += s
    print(source_sentence)
    source_wordcloud = WordCloud(width=800, height=800, max_words=100, background_color='white').generate_from_text(source_sentence)
    

            
            
    target_sentence = ""
    for i,d_ID in enumerate(unlabeled_target.data_ID[:100]):
        temp_dict = unlabeled_target.data[d_ID]
        temp_dict["text"] = [s.split(" ") for s in temp_dict["sentence"]]
#         print(temp_dict["sentence"])
        for s in temp_dict["sentence"]:
            target_sentence += s
            
    target_wordcloud = WordCloud(width=800, height=800, max_words=100, background_color='white').generate_from_text(target_sentence)
    
    plt.figure(figsize=(8, 8), facecolor=None)
    plt.imshow(source_wordcloud)
    plt.axis("off")
    plt.tight_layout(pad=0)
    plt.savefig("source.png")
       
    plt.figure(figsize=(8, 8), facecolor=None)
    plt.imshow(target_wordcloud)
    plt.axis("off")
    plt.tight_layout(pad=0)
