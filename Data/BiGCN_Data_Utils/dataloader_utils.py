...
class RumorLoader(Dataset):
    ...
    def split(self, percent=[0.5, 1.0]):
        ...
        for i in range(len(percent)):
            ...
            rst[i].instance_weights = torch.ones([len(rst[i].data_ID)], dtype=torch.float32, device=torch.device('cuda'))
        return rst
    ...
    def select(self, idxs):
        ...
        obj.instance_weights = torch.ones([len(obj.data_ID)], dtype=torch.float32, device=torch.device('cuda'))
        obj._confidence = torch.ones(len(idxs),device=torch.device('cuda'))
        obj._entrophy = torch.zeros(len(idxs),device=torch.device('cuda'))
        obj.read_indexs = np.arange(len(idxs))
        return obj
