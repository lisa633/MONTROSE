...
from TrainingEnv import BaseTrainer, GradientReversal, VirtualModel, CustomDataset, AugmentationModel
import torch.nn.functional as F, numpy as np

class BiATTrainEnv(BaseTrainer,AugmentationModel):
...
class BiATTrainer(BiATTrainEnv):
    ...
    def EVatLoss(self, model, classifier, batch):
        ...
        pooledOutput = model.AugBatch2Vecs(batch)
    ...
    def TLoss(self, HNet:VirtualModel, TClassifier:nn.Module, batch):
        # return HNet.auglossAndAccV2(TClassifier, batch, temperature=self.temperature)
        return self.augLossAndAccV2(HNet,TClassifier, batch, temperature=self.temperature,aug_type="adver")
    ...
    def dataSampler(self, labeledSource, labeledTarget, unlabeledTarget):
        ...
        def sample_fn(max_iter_step):
            for _ in range(max_iter_step):
                ...
                batchCE = labeledSource.collate_fn(items1[:bs1] + items2[:bs2])
                batchEVat = unlabeledTarget.collate_fn(items3)
                batchTNet = labeledTarget.collate_fn(items2)
                batchAAT = labeledSource.collate_fn(items1)
    ...
    def Training(self, HNet:nn.Module, HClassifier, TClassifier, labeledSource : Dataset, labeledTarget : Dataset,
                    unlabeledTarget : Dataset, validSet : Dataset, testSet:Dataset,
                        testLabel=None, maxStep=10000, validEvery=20,grad_accum_cnt=1, learning_rate=2e-5):
        assert labeledTarget is not None
        print("labeled Source/labeled Target/unlabeled Target: {}/{}/{}".format(len(labeledSource),
                                                                                len(labeledTarget),
                                                                                len(unlabeledTarget)))
        optim_N = HNet.obtain_optim(learning_rate * 1.0 / grad_accum_cnt)
        ...
        for step, (batchCE, batchAAT, batchEVat, batchTNet) in enumerate(sampler(maxStep)):
            ...
            if (step + 1) % validEvery == 0:
                ...
                if acc > self.best_valid_acc:
                    self.valid(HNet, testSet, testLabel, suffix=f"TestPerf_{self.suffix}")
                    # torch.save(HNet.state_dict(), self.model_file)
                    HNet.save_model(self.model_file)
                    self.best_valid_acc = acc
                else:
                    self.valid(HNet, testSet, testLabel, suffix=f"Perf_{self.suffix}")
